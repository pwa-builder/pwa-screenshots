...
router.post('/post', async function(request: express.Request, response: express.Response){ 
    ...
    const screenshotFullScreen = await createTemporaryFile('screenshotFullScreen', '.png');
    const screenshotPhone = await createTemporaryFile('screenshotPhone', '.png');
    await generateScreenshots(request.query.url, screenshotFullScreen, screenshotPhone);
    archive.pipe(output);
  ...
  async function generateScreenshots(url, pathToFullPageScreenshot, pathToPhoneScreenshot){
